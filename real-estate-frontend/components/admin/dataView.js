...
const DataView = ({ data, handleOptionClick }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get("http://localhost:9000/getUsers", { withCredentials: true });
        setTotalUsers(response.data); // Array of properties
        console.log("response her hai", response.data);
      } catch (error) {
        console.error("Error fetching properties:", error);
      }
    };
    ...
    <>
      <div className='bg-gray-50 w-full flex justify-center items-center'>
        <div className='w-4/5 h-4/5'>
          {data === 'Dashboard' ? <Dashboard totalUsers={totalUsers}/> : (null)}
